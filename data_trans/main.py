...
def merge_runner(
    input_dir, output_dir, merged_output_dir, output_folder, metric_output_path
):
...
def merge_abnormal_normal_to_overall(prefix_dir):
    ...
    )  # Create the 'overall' directory if it doesn't exist
    ...
    for sub_dir in sub_dirs:
        ...
        abnormal_files = set(
            f.name for f in abnormal_sub_dir.glob("*.csv") if f.is_file()
        )
...
def main():
    ...
    for prefix_dir in root_base_dir.iterdir():
        if prefix_dir.is_dir():
            ...
            subdir_name = (
                prefix_dir.name
            )  # Use the name of the subdirectory to create output paths
            output_dir_sub = output_base_dir / subdir_name
            output_dir_sub.mkdir(
                parents=True, exist_ok=True
            )  # Ensure subdirectory exists in output base
