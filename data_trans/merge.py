...
def process_data_merge(df):
    ...
    # 每五行分组，计算均值
    result = (
        df.groupby(df.index // 12)
        .agg(
            {
                "TimeUnix": "first",  # 取每组中的第一个时间戳
                "k8s.pod.cpu.usage": "mean",
                "k8s.pod.cpu_limit_utilization": "mean",
        ...
        if filename.endswith(".csv"):
            ...
            if processed_file and os.path.exists(processed_file):
                ...
                merged_data = pd.concat(
                    [output_df, processed_df.drop("TimeUnix", axis=1)], axis=1
                )
                ...
                print(
                    f"跳过文件: {output_file}，因为 processed_metrics 目录下没有匹配的文件"
                )
...
def process_csv_file_merge(input_path, output_path):
    ...
    rename_mapping = {
        ...
        "k8s.pod.cpu_limit_utilization": "CpuUsageRate(%)",
