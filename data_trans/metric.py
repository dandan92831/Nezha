    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            if "BucketCounts" in df:
                ...
                # 计算延迟百分比
                df["Latency_P90"] = df["BucketCounts"].apply(
                    lambda x: process_bucket_counts(x, 0.10)
                )
                df["Latency_P95"] = df["BucketCounts"].apply(
                    lambda x: process_bucket_counts(x, 0.05)
                )
                df["Latency_P99"] = df["BucketCounts"].apply(
                    lambda x: process_bucket_counts(x, 0.01)
                )
...
def add_latency_columns(df):
    ...
    df.loc[server_mask, ["server_P90", "server_P95", "server_P99"]] = df.loc[
        server_mask, ["P90", "P95", "P99"]
    ].values
    df.loc[client_mask, ["client_P90", "client_P95", "client_P99"]] = df.loc[
        client_mask, ["P90", "P95", "P99"]
    ].values
    return df
...
def main(input_file, output_folder):
    ...
    for filename in os.listdir(output_folder):
        if filename.endswith(".csv"):
            ...
            # 计算加权和
            df["P90"] = (
                df["Latency_P90"].apply(lambda x: calculate_weighted_sum(x, weights))
                / 10
            )
            df["P95"] = (
                df["Latency_P95"].apply(lambda x: calculate_weighted_sum(x, weights))
                / 5
            )
            df["P99"] = df["Latency_P99"].apply(
                lambda x: calculate_weighted_sum(x, weights)
            )
